...
// Forward declaration so Node can friend operator<< that mentions DictionaryList
...
class Node {
  ...
  // allow operator<< access to Node internals through friendship
  friend ostream& operator<<(ostream& out, const DictionaryList& dl);
private:
  Key keyM;
  Datum datumM;
  Node *nextM;
